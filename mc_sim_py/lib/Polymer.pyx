...
This is a version of Polymer.py converted to pyx versions
...
cdef class Chain(object):
    ...
    cdef public positions_list
    cdef public _tmp
    
    ## public
    cdef public chain
    cdef public _monomer_types
    cdef public _chain_structure
    cdef public object box
    cdef public float total_energy
    cdef public name
    cdef public int idx
    ##
    cdef public last_values
    ...
    cdef public move_name
    cdef public type_of_move

    ## simple stats
    cdef public float min_energy
    cdef public float min_rg
    ...
    cpdef bint move(self, int type_of_move):
        """
    ...
    cpdef bint valid_chain(self, bint assertion=True):
        """
        ...
        for idx in xrange(1, self._total_length - 1):
    ...
    cpdef get_direction(self, monomer_b, monomer_a):
        """
    ...
    cpdef bint end_rotate(self):
        """
        rotate with begin
        """
        cdef int random_direction = randrange(0, Lattice.z)
        new_position = self.chain[-2].neighbours_position[random_direction]
        #new_position = Lattice.get_coordinate(self.chain[-2]._position, random_direction)
        ...
        new_monomer = copy.copy(self.chain[-1])
        new_monomer.position = new_position
        new_monomer._open = Lattice.get_open_coordinate(self.chain[-2]._open, random_direction)
        
        cdef bint perform_move = True
        cdef float new_energy = 0.0
        cdef float dE = 0.0
        cdef float current_energy

        if self.box.athermal_state == False:
            current_energy = self.chain[-1].energy ## energy of one monomer
            new_energy = self.box.calculate_monomer_energy(new_monomer, exclude_positions = self.chain[-1]._position)
        
            dE = new_energy - current_energy
        
            if self.box.accept(dE, new_position):
                perform_move = True
            else:
                perform_move = False
        ...
        if perform_move:
            """
            if not new_energy or not dE:
                current_energy = self.chain[-1].energy ## energy of one monomer
                new_energy = self.box.calculate_monomer_energy(new_monomer, exclude_positions = self.chain[-1]._position)
                dE = new_energy - current_energy
            
            if dE != 0.0:
                self.box.global_energy_update()
            """
            new_monomer.energy = new_energy
    ...
    cpdef bint begin_rotate(self):
        ...
        new_monomer = copy.copy(self.chain[0])
        new_monomer.position = new_position
        new_monomer._open = Lattice.get_open_coordinate(self.chain[1]._open, random_direction)
        #new_monomer.next_direction = Lattice.TURN_DIRECTION[random_direction]
        ...
        if self.box.athermal_state == False:
            ...
            current_energy = self.chain[0].energy
            new_energy = self.box.calculate_monomer_energy(new_monomer, exclude_positions = self.chain[0]._position)
    ...
    cpdef bint begin_snake(self):
        return self._snake(0)
    
    cpdef bint end_snake(self):
        return self._snake(1)
    
    cpdef bint _snake(self, int move_type):
        """
        Snake move
        """
        ...
        if is_occupied(new_position) or new_position in self.positions_list:
            #logging.debug("Reject snake; Position: (%d, %d, %d) is occupied" % (new_position))
            return False
            
        # make copy of old monomer, make operation on this chain cause we need to update
        # position of every monomer on chain
        #new_chain = copy.deepcopy(self)

        old_position_list = self.positions_list[:]  #[ x._position for x in self.chain ]
        #old_positions_list_orig = self.positions_list[:]
        old_open_position_list = [ x._open for x in self.chain ]
        old_next_direction_list = [ x.next_direction for x in self.chain ]

        cdef float old_energy = self.total_energy
        #new_chain.box = self.box
        #new_chain.positions_list = []
        self.positions_list = []
        ...
        if move_type == 0: # left
            ...
            for idx in xrange(1, self._total_length):
            ...
            for idx in xrange(0, self._total_length - 1):
                self.chain[idx].position = old_position_list[idx + 1]
                self.chain[idx]._open = old_open_position_list[idx + 1]
                self.chain[idx].next_direction = old_next_direction_list[idx + 1]
        ...
        cdef float new_energy = self.box.calculate_chain_energy(self)
        cdef float dE = new_energy - old_energy
    ...
    cpdef bint segment_rotate(self):
        """
        ...
        cdef int monomer_idx = randrange(1, self._total_length - 1)
        ...
        cdef int number_of_positions = len(common_neighbours_positions)
        ...
        new_monomer = copy.copy(rotate_monomer)
        new_monomer.position = new_position
        new_monomer._open = open_position
        
        cdef float current_energy = self.box.calculate_monomer_energy(rotate_monomer)
        cdef float new_energy = self.box.calculate_monomer_energy(new_monomer, exclude_positions = rotate_monomer._position)
        
        #print self.total_energy - current_energy + new_energy, self.total_energy
        
        cdef float dE = new_energy - current_energy
        ...
        if accept:
            ...
            self[monomer_idx] = new_monomer
            self.positions_list[monomer_idx] = new_position
            self.chain[monomer_idx-1].next_direction = self.chain[monomer_idx-1].neighbours_position.index(new_position)
            self.chain[monomer_idx].next_direction = self.chain[monomer_idx].neighbours_position.index(self.chain[monomer_idx+1]._position)
    ...
    cpdef bint pull_move(self):
        """
        Pull move, N. Lesh, M. Mitzemacher, S. Whitesides
        """
        ## flags
        cdef bint valid_configuration = False
        if settings.DEBUG: self.valid_chain()
        #if monomer_idx is None:
        cdef int monomer_idx = randrange(0, self._total_length - 1)
        ## two special cases at the begin/end
        cdef int case = 3
        cdef float old_energy
        ...
        if monomer_idx == 0:
            ...
            for idx in xrange(2, self._total_length):
                ...
                if is_neighbour(i_monomer._open, p_monomer._open):
            ...
            case = 0
            if settings.DEBUG: self.valid_chain()
        elif monomer_idx == self._total_length - 1:
            dir_1 = randrange(0, Lattice.z)
            dir_2 = randrange(0, Lattice.z)
            ...
            for idx in xrange(self._total_length - 2, -1, -1):
                ...
                if is_neighbour(i_monomer._open, n_monomer._open):
        ...
        if case == 3: 
            ...
            if sub_case == 0:
                ...
                Lnext_direction = self.box.lattice.get_neighbours(L_position)[0].index(next_monomer._position)
            elif sub_case == 1:
                ...
                Lprev_direction = self.box.lattice.get_neighbours(L_position)[0].index(prev_monomer._position)
            ...
            if not is_neighbour(L_position, C_position):
                return False
            ...
            # direction L->C
            C_nb = self.box.lattice.get_neighbours(C_position)[0]
            ...
            if not valid_configuration:
                if sub_case == 0:
                    for j in xrange(monomer_idx - 2, -1, -1):
                        ...
                        if is_neighbour(j_monomer._open, i_monomer._open):
        ...
        if accept_move is True:
            ...
            for idx in monomer_list:
                ...
                if m._position != old_pos or m._next_direction != old_dir:
    ...
    def calculate_values(self):
        ...
        self.last_values = rg + [distance, self.total_energy]
    ...
    def __setitem__(self, key, value):
        ...
        ## update chain start/stop values 
